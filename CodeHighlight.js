(function ($) {
	$.fn.codeHighlight = function (options) {
		...
		addElementsArray = $.grep(addElementsArray, function (n) {
		...
		addAttrArray = $.grep(addAttrArray, function (n) {
			return $.trim(n).length > 0;
		});
		...
		var initStyle = setInterval(function () {
			if ($(settings.codePre).find(".elem").length != 0 || $(settings.codePre).find(".attr").length != 0 || $(settings.codePre).find(".str").length != 0 || $(settings.codePre).find(".plain").length != 0) {
				$(settings.codePre).find(".elem").css({
					"color": settings.elemColor,
					"font-weight": "bold"
				});
				$(settings.codePre).find(".attr").css({
					"color": settings.attrColor
				});
				$(settings.codePre).find(".str").css({
					"color": settings.strColor
				});
				$(settings.codePre).find(".plain").css({
					"color": settings.plainColor
				});
				//特殊 element 标签样式
				$(settings.codePre).find(".elem.js").css({
					"color": settings.jsColor != "" ? settings.jsColor : settings.elemColor
				});
				$(settings.codePre).find(".elem.css").css({
					"color": settings.cssColor != "" ? settings.cssColor : settings.elemColor
				});
				$(settings.codePre).find(".elem.meta").css({
					"color": settings.metaColor != "" ? settings.metaColor : settings.elemColor
				});
				//特殊的 attribute 属性样式
				$(settings.codePre).find(".attr.id").css({
					"color": settings.idColor != "" ? settings.idColor : settings.attrColor
				});
				$(settings.codePre).find(".attr.cla").css({
					"color": settings.claColor != "" ? settings.claColor : settings.attrColor
				});
				$(settings.codePre).find(".attr.sty").css({
					"color": settings.styColor != "" ? settings.styColor : settings.attrColor
				});
